...
pp.add_argument('--load-path', type=str, default='.', help='Path to a directory containing a copy of the CUB dataset.')
...
images_df = pd.read_csv(
    os.path.join(args.load_path, 'images.txt'),
    delimiter = ' ',
    header = None,
...
splits_df = pd.read_csv(
    os.path.join(args.load_path, 'train_test_split.txt'),
    delimiter = ' ',
    header = None,
...
attributes_df = pd.read_csv(
    os.path.join(args.load_path, 'attributes', 'image_attribute_labels.txt'),
    delimiter = ' ',
    header = None,
