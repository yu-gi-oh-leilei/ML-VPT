...
def init_logeger(config):
    ...
    if dist.get_rank() == 0:
        ...
        os.makedirs(os.path.join(config.INPUT_OUTPUT.output, 'tmpdata'), exist_ok=True)
...
def load_model_prompt(cfg, logger, model):
    if cfg.INPUT_OUTPUT.resume_prompt:
        if os.path.isfile(cfg.INPUT_OUTPUT.resume_prompt):
            logger.info("=> loading checkpoint '{}'".format(cfg.INPUT_OUTPUT.resume_prompt))
            checkpoint = torch.load(cfg.INPUT_OUTPUT.resume_prompt, map_location=torch.device(dist.get_rank()))
            ...
            for k, v in state_dict.items():
                if k in model_dict and v.shape == model_dict[k].shape and 'prompt_learner' in k and 'ctx' in k:
                # if k in model_dict and v.shape == model_dict[k].shape and 'prompt_learner' not in k:
                    model_dict[k] = v
                    logger.info('\t Load layers: {}'.format(k))
            # model.module.load_state_dict(state_dict, strict=False)
            model.module.load_state_dict(model_dict, strict=False)
            # model.module.load_state_dict(checkpoint['state_dict'])
            logger.info("=> loaded checkpoint '{}' (epoch {})"
                  .format(cfg.INPUT_OUTPUT.resume_prompt, checkpoint['epoch']))
...
def load_model_transformer(cfg, logger, model):
    if cfg.INPUT_OUTPUT.resume_transformer:
        if os.path.isfile(cfg.INPUT_OUTPUT.resume_transformer):
            logger.info("=> loading checkpoint '{}'".format(cfg.INPUT_OUTPUT.resume_transformer))
            checkpoint = torch.load(cfg.INPUT_OUTPUT.resume_transformer, map_location=torch.device(dist.get_rank()))
            ...
            for k, v in state_dict.items():
                if k in model_dict and v.shape == model_dict[k].shape:
                    if 'transformer.decoder' in k or 'semantic_query' in k:
                        model_dict[k] = v
                        logger.info('\t Load layers: {}'.format(k))
            # model.module.load_state_dict(state_dict, strict=False)
            model.module.load_state_dict(model_dict, strict=False)
            # model.module.load_state_dict(checkpoint['state_dict'])
            logger.info("=> loaded checkpoint '{}' (epoch {})"
                  .format(cfg.INPUT_OUTPUT.resume_transformer, checkpoint['epoch']))
...
def load_model(cfg, logger, model):
    if cfg.INPUT_OUTPUT.resume:
        if os.path.isfile(cfg.INPUT_OUTPUT.resume):
            ...
            # del model_dict['vis_prototype_pos']
            # del model_dict['vis_prototype_neg']
...
def get_ema_co(cfg):
    import numpy as np
    if "coco" in cfg.DATA.dataname:
        ema_co = np.exp(np.log(0.82)/(641*cfg.TRAIN.ratio))  # type: ignore
        # ema_co = 0.9997
    elif "nus" in cfg.DATA.dataname:
        ema_co = np.exp(np.log(0.82)/(980*cfg.TRAIN.ratio))  # type: ignore
        # ema_co = 0.9998
    elif "voc" in cfg.DATA.dataname:
        ema_co = np.exp(np.log(0.82)/(39*cfg.TRAIN.ratio))  # type: ignore
        # ema_co = 0.9956
    elif "vg500" in cfg.DATA.dataname:
        ema_co = np.exp(np.log(0.82)/(647*cfg.TRAIN.ratio))  # type: ignore
        # ema_co = 0.9997
    elif "CUB" or 'cub' in  cfg.DATA.dataname:
        if cfg.OPTIMIZER.batch_size == 96:
            ema_co = np.exp(np.log(0.82)/(63*cfg.TRAIN.ratio))
        else:
            ema_co = np.exp(np.log(0.82)/(47*cfg.TRAIN.ratio))  # type: ignore
    else:
        assert(False)
    return float(ema_co)
...
#         self._update(model, update_fn=lambda e, m: m)
            ...
            #         del model_dict[k]
                    ...
                    # elif cfg.INPUT_OUTPUT.dataname in ('voc2007', 'voc2012') and k in model_dict and v.shape != model_dict[k].shape: # and k in ('query_embed.weight', 'fc.W', 'fc.b'):
                #     # print('============================')
                #     if 'semantic_query.weight' in k or 'fc.W' in k or 'fc.b' in k:
                #         voc_from_coco = [4, 1, 14, 8, 39, 5, 2, 15, 56, 19, 60, 16, 17, 3, 0, 58, 18, 57, 6, 62]
                #         new_v = torch.zeros_like(model_dict[k])
                #         for key, value in enumerate(voc_from_coco):
                #             # print(k)
                #             if 'semantic_query.weight' in k:
                #                 new_v[key] = v[value]
                #             elif 'fc.W' in k:
                #                 new_v[:, key, :] = v[:, value, :]
                #             elif 'fc.b' in k:
