...
def asl_mAP(targs, preds, return_each=True):
...
# def asl_mAP(imagessetfilelist, num, return_each=False, logger=None, dataname='wider'):
#     """Returns the model's average precision for each class
#     Return:
#         ap (FloatTensor): 1xK tensor, with avg precision for each class k
#     """
...
#     # torch.cat((preds, targs), 1)
#     preds = seg[:,:num].astype(np.int32) # preds
#     targs = seg[:,num:].astype(np.int32) # label
...
def voc_mAP(imagessetfilelist, num,return_each=False, logger=None, dataname='wider'):
    if isinstance(imagessetfilelist, str):
        imagessetfilelist = [imagessetfilelist]
    lines = []
    for imagessetfile in imagessetfilelist:
    ...
    all_threshold = 0.50
    top3_threshold = 0.50
    overall(seg[:,:num] ,gt_label, all_threshold, logger=logger)
    overall_topk(seg[:,:num] ,gt_label, top3_threshold, 3, logger=logger)
...
def overall_topk(output, gt_label, threshold, k=3, logger=None):    
    ...
    if logger is not None:
        logger.info(' *   CP3    CR3    CF3    OP3    OR3    OF3 ')
        logger.info(' *  {:.2f}  {:.2f}  {:.2f}  {:.2f}  {:.2f}  {:.2f}'.format(mean_c_p, mean_c_r, mean_c_f, o_p, o_r, o_f))
    else:
...
def overall(output, gt_label, threshold, logger=None):
