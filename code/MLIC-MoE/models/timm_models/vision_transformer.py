...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.attn_mask is None:
            self.attn_mask = torch.ones(attn.size(2), attn.size(3), dtype=torch.bool).to(attn.device)
            self.attn_mask[1:self.num_aux_tokens+1, 1:self.num_aux_tokens+1] = False
            for i in range(self.num_aux_tokens+1):
                self.attn_mask[i, i] = True


        if self.attn_mask.size() != attn.size():
            # print("attn_mask size: ", self.attn_mask.size())
            attn_mask = self.attn_mask.unsqueeze(0).unsqueeze(0).expand(attn.size(0), attn.size(1), -1, -1)
            attn.masked_fill_(attn_mask.logical_not(), float('-inf'))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer, cfg=cfg)
    ...
    def forward(self, x):
        x_norm, hidden_states = self.forward_features(x)
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, cfg=None, **kwargs):
...
# def convert(arch, pos_embed):
#     state_dict = model_zoo.load_url(torchvision_urls[arch])
#     timm_state_dict = {}
...
#             name = 'pos_embed'
#             param = resize_pos_embed(param, pos_embed)
...
def vit_base_patch16_224(pretrained=False, cfg=None, **kwargs):
    """ ViT-Base (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224', cfg=cfg, pretrained=pretrained, **model_kwargs)
    # state_dict = convert('vit_base_patch16_224', model.pos_embed)
    # model.load_state_dict(state_dict, strict=True)
    # print('successfully loaded torchvision models!')
    return model


@register_backbone(feat_dim=1024)
def vit_large_patch16_224(pretrained=False, cfg=None, **kwargs):
    """ ViT-Large model (ViT-L/32) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-1k weights fine-tuned from in21k @ 224x224, source https://github.com/google-research/vision_transformer.
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_224', cfg=cfg, pretrained=pretrained, **model_kwargs)
    # state_dict = convert('vit_large_patch16_224', model.pos_embed)
    # model.load_state_dict(state_dict, strict=True)
    # print('successfully loaded torchvision models!')
    return model
...
@register_backbone(feat_dim=768)
def vit_base_patch16_224_in21k(pretrained=False, cfg=None, **kwargs):
    """ ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    model = _create_vision_transformer('vit_base_patch16_224_in21k', cfg=cfg, pretrained=pretrained, **model_kwargs)
    return model


@register_backbone(feat_dim=1024)
def vit_large_patch16_224_in21k(pretrained=False, cfg=None, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    ImageNet-21k weights @ 224x224, source https://github.com/google-research/vision_transformer.
    NOTE: this model has valid 21k classifier head and no representation (pre-logits) layer
    """
    model_kwargs = dict(patch_size=16, embed_dim=1024, depth=24, num_heads=16, **kwargs)
    model = _create_vision_transformer('vit_large_patch16_224_in21k', cfg=cfg, pretrained=pretrained, **model_kwargs)
    return model
