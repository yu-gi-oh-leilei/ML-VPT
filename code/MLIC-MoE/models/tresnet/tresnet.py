...
def _tresnet(arch, layers, pretrained, **kwargs):
    class_code = TResNet.__init__.__code__
    class_args = class_code.co_varnames[1:class_code.co_argcount]
    for k in list(kwargs):
        if k not in class_args:
            kwargs.pop(k)
    model = TResNet(layers=layers, **kwargs)
    logger.info("backbone params are initialized by Pytorch official model")
    state_dict = model_zoo.load_url(model_urls[arch], map_location='cpu')['model']
    state_dict = {k:v for k, v in state_dict.items() if k in model.state_dict() and 'head.fc' not in k}
    model.load_state_dict(state_dict, strict=True)
    return model


@register_backbone(feat_dim=2432)
def tresnet_m(pretrained, cfg=None, **kwargs):
    """ Constructs a medium TResnet model.
    """
    arch = 'tresnet_m_{}'.format(kwargs['img_size'])
    return _tresnet(arch, [3, 4, 11, 3], pretrained)


@register_backbone(feat_dim=2432)
def tresnet_l(pretrained, cfg=None, **kwargs):
    """ Constructs a large TResnet model.
    """
    arch = 'tresnet_l_448'
    return _tresnet(arch, [4, 5, 18, 3], pretrained, width_factor=1.2)


@register_backbone(feat_dim=2432)
def tresnet_xl(pretrained, cfg=None, **kwargs):
    """ Constructs an extra-large TResnet model.
    """
    arch = 'tresnet_xl_{}'.format(kwargs['img_size'])
    return _tresnet(arch, [4, 5, 24, 3], pretrained, width_factor=1.3)
